exports.seed = async knex => {
  ...
  await knex('SIS_PERMISOS').insert([
  ...
  const permisos = await knex.select('IDEN_PERMISO', 'CODI_PERMISO').from('SIS_PERMISOS')
  const roles = await knex.select('IDEN_ROL', 'CODI_ROL').from('SIS_ROLES')
  await knex('SIS_PERMISOS_ROLES').insert([
    // Clientes - Visualizar
