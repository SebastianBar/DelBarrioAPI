...
import { genHash } from '../../auth/_helpers.js';
...
const validate = async (model) => {
  ...
  const verifiedMail = ((typeof model.attributes.IDEN_USUARIO === 'undefined') || model.attributes.EMAIL_USUARIO !== model._previousAttributes.EMAIL_USUARIO);
  if (verifiedMail) {
    if (validations.EMAIL_USUARIO.length === 2) validations.EMAIL_USUARIO.push(mailComparison);
  } else if (validations.EMAIL_USUARIO.length === 3) validations.EMAIL_USUARIO.pop();
