exports.up = async (knex) => {
  ...
  const createSisPermisos = () => knex.schema.createTableIfNotExists('SIS_PERMISOS', (table) => {
    table.increments('IDEN_PERMISO').unsigned().primary();
    table.integer('CODI_PERMISO').notNull().unique();
    table.string('NOMB_PERMISO').notNull();
  });

  const createSisPermisosRoles = () => knex.schema.createTableIfNotExists('SIS_PERMISOS_ROLES', (table) => {
  ...
  const createUsrUsuarios = () => knex.schema.createTableIfNotExists('USR_USUARIOS', (table) => {
  ...
  const createPerTelefonos = () => knex.schema.createTableIfNotExists('PER_TELEFONOS', (table) => {
  ...
  const createPerRubros = () => knex.schema.createTableIfNotExists('PER_RUBROS', (table) => {
    table.increments('IDEN_RUBRO').unsigned().primary();
    table.string('NOMB_RUBRO', 100).notNull();
    table.boolean('FLAG_VIGENTE').notNull().defaultTo(true);
  });

  const createPerEmprendedores = () => knex.schema.createTableIfNotExists('PER_EMPRENDEDORES', (table) => {
  ...
  const createPerPersonas = () => knex.schema.createTableIfNotExists('PER_PERSONAS', (table) => {
  ...
  const createReqCategorias = () => knex.schema.createTableIfNotExists('REQ_CATEGORIAS', (table) => {
  ...
  const createReqPublicaciones = () => knex.schema.createTableIfNotExists('REQ_PUBLICACIONES', (table) => {
  ...
  const createReqImagenes = () => knex.schema.createTableIfNotExists('REQ_IMAGENES', (table) => {
  ...
  const createReqOfertas = () => knex.schema.createTableIfNotExists('REQ_OFERTAS', (table) => {
  ...
  const createReqEtiquetas = () => knex.schema.createTableIfNotExists('REQ_ETIQUETAS', (table) => {
    table.increments('IDEN_ETIQUETA').unsigned().primary();
    table.string('NOMB_ETIQUETA').notNull();
  });

  const createReqPublicacionesEtiquetas = () => knex.schema.createTableIfNotExists('REQ_PUBLICACIONES_ETIQUETAS', (table) => {
  ...
  const createReqComentarios = () => knex.schema.createTableIfNotExists('REQ_COMENTARIOS', (table) => {
  ...
  const createReqRespuestas = () => knex.schema.createTableIfNotExists('REQ_RESPUESTAS', (table) => {
  ...
  const createReqCalificaciones = () => knex.schema.createTableIfNotExists('REQ_CALIFICACIONES', (table) => {
  ...
  const createReqMotivosDenuncia = () => knex.schema.createTableIfNotExists('REQ_MOTIVOS_DENUNCIA', (table) => {
    table.increments('IDEN_MOTIVO_DENUNCIA').unsigned().primary();
    table.string('NOMB_MOTIVO_DENUNCIA', 100).notNull();
    table.boolean('FLAG_VIGENTE').notNull().defaultTo(true);
  });

  const createReqDenuncias = () => knex.schema.createTableIfNotExists('REQ_DENUNCIAS', (table) => {
  ...
  const createReqResolucionDenuncias = () => knex.schema.createTableIfNotExists('REQ_RESOLUCION_DENUNCIAS', (table) => {
  ...
  const createReqFaq = () => knex.schema.createTableIfNotExists('REQ_FAQ', (table) => {
    table.increments('IDEN_FAQ').unsigned().primary();
    table.string('NOMB_FAQ', 255).notNull();
    table.text('DESC_FAQ', 1000).notNull();
  });

  const createReqMotivosDeshabilitacion = () => knex.schema.createTableIfNotExists('REQ_MOTIVOS_DESHABILITACION', (table) => {
    table.increments('IDEN_MOTIVO_DESHABILITACION').unsigned().primary();
    table.string('NOMB_MOTIVO_DESHABILITACION', 100).notNull();
    table.boolean('FLAG_VIGENTE').notNull().defaultTo(true);
  });

  const createReqDeshabilitacionesCuentas = () => knex.schema.createTableIfNotExists('REQ_DESHABILITACION_CUENTAS', (table) => {
